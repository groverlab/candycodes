    ...
    def triangulate(self):
        ...
        for center in self.candies:
            ...
            if on_edge and remove_edges:
                ...
                # If we get this far, the code is good and can be kept.
                ...
                # additions to save the code as an integer:
                int_code = 0
                for i, c in enumerate(code_string):  # for each character in the string:
                    int_code += color_number[c] * (2 ** (4 * i))
                self.ints.append(int_code)
...
for i1, s1 in enumerate(candycodes):
    for i2, s2 in enumerate(candycodes):
        if i2 > i1:   # just do a diagonal half and no self-compares
            # print("Comparing candycode %s with candycode %s" % (s1, s2))
